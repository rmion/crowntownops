  ...
  var app = new Vue({
    ...
    methods: {
        ...
        fetchGSheetData(weekday) {
            if (weekday !== undefined) {
              localStorage.removeItem('destinations');localStorage.removeItem('route');localStorage.removeItem('stopNumber');
              this.allMarkers = []
              this.resetMap();
            }
            if (localStorage.getItem('destinations') && JSON.parse(localStorage.getItem('destinations')).updated === this.TODAY.toLocaleDateString()) {
              ...
              this.hydrateApp(JSON.parse(localStorage.getItem('destinations')).destinations)
                ...
                fetch(`https://sheetdb.io/api/v1/65s1qbqcffqpa/search?Service-Day=${weekday == undefined ? this.dayOfWeek : weekday}`, {
                        headers: headers
                    })
